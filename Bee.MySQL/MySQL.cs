...
namespace Bee.MySQL
{
    public class MySQL
    {
                ...
                using (MySqlConnection connection = new MySqlConnection(connectionString))
                {
                    ...
                    using (MySqlTransaction transaction = connection.BeginTransaction())
                    {
                        ...
                        catch (MySqlException e)
                        {
                            ...
                            return new Insert { execute = false, message = "Transaction canceled. " + e.Message, duplicate = (e.Number == 1062) ? true : false };
                        }
                        catch (Exception e)
                        {
                            transaction.Rollback();
        ...
        public static ExecuteQuery executeQuery(string connectionString, string procedureName, Dictionary<string, object> procedureParameters = null)
        {
            ...
            {
                return new ExecuteQuery { execute = false, message = "Request failed. " + e.Message, duplicate = (e.Number == 1062) ? true : false };
            }
            catch (Exception e)
            {
